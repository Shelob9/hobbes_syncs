var hobbes_syncs_canvas = false,
	hsyncc_get_config_object,
	hsyncc_record_change,
	hsyncc_canvas_init,
	hsyncc_get_default_setting,
	hsyncc_code_editor,
	init_magic_tags,
	hsyncc_rebuild_magics,
	config_object = {},
	magic_tags = [];
...
jQuery( function($){
	...
	init_magic_tags = function(){
		...
		magicfields.each(function(k,v){
			...
			if(input.hasClass('hobbes-syncs-conditional-value-field')){
				wrapper.width('auto');
			}
	...
	hsyncc_get_config_object = function(el){
		// new sync first
		$('#hobbes_syncs-id').trigger('change');
		var clicked 	= $(el),
			config 		= $('#hobbes-syncs-live-config').val(),
			required 	= $('[required]'),
			clean		= true;

		for( var input = 0; input < required.length; input++ ){
			if( required[input].value.length <= 0 && $( required[input] ).is(':visible') ){
				$( required[input] ).addClass('hobbes-syncs-input-error');
				clean = false;
			}else{
				$( required[input] ).removeClass('hobbes-syncs-input-error');
			}
		}
		if( clean ){
			hobbes_syncs_canvas = config;
		}
		clicked.data( 'config', config );
		return clean;
	}

	hsyncc_record_change = function(){
		// hook and rebuild the fields list
		jQuery(document).trigger('record_change');
		jQuery('#hobbes_syncs-id').trigger('change');
		jQuery('#hobbes-syncs-field-sync').trigger('refresh');
	}
	
	hsyncc_canvas_init = function(){

		if( !hobbes_syncs_canvas ){
			// bind changes
			jQuery('#hobbes-syncs-main-canvas').on('keydown keyup change','input, select, textarea', function(e) {
				config_object = jQuery('#hobbes-syncs-main-form').formJSON(); // perhaps load into memory to keep it live.
				jQuery('#hobbes-syncs-live-config').val( JSON.stringify( config_object ) ).trigger('change');
			});
			// bind editor
			hsyncc_init_editor();
			hobbes_syncs_canvas = jQuery('#hobbes-syncs-live-config').val();
			config_object = JSON.parse( hobbes_syncs_canvas ); // perhaps load into memory to keep it live.
		}
		...
		if( $('.hobbes-syncs-group-wrapper').length ){
			$( ".hobbes-syncs-group-wrapper" ).sortable({
				handle: ".dashicons-sort",
				update: function(){
					jQuery('#hobbes_syncs-id').trigger('change');
				}
			});
			$( ".hobbes-syncs-fields-list" ).sortable({
				handle: ".dashicons-sort",
				update: function(){
					jQuery('#hobbes_syncs-id').trigger('change');
				}
	...
	hsyncc_get_default_setting = function(obj){

		var id = 'node_' + Math.round(Math.random() * 99887766) + '_' + Math.round(Math.random() * 99887766),
			new_object = {},
			//config_object = JSON.parse( jQuery('#hobbes-syncs-live-config').val() ), // perhaps load into memory to keep it live.
		...
		jQuery('#hobbes-syncs-live-config').val( JSON.stringify( config_object ) );
		jQuery('#hobbes-syncs-field-sync').trigger('refresh');
	}
	...
	hsyncc_rebuild_magics = function(){
		...
		$( ".magic-tag-enabled" ).bind( "keydown", function( event ) {
			...
			source: function( request, response ) {
				...
				if( config_object.search_form && config_object.form_fields ){
					...
					category = $('#hobbes-syncs-label-tags').text();
	...
	// trash 
	$(document).on('click', '.hobbes-syncs-card-actions .confirm a', function(e){
		e.preventDefault();
		var parent = $(this).closest('.hobbes-syncs-card-content');
	...
	// bind tabs
	$(document).on('click', '.hobbes-syncs-nav-tabs a', function(e){
		...
		$('.hobbes-syncs-nav-tabs .current').removeClass('current');
		$('.hobbes-syncs-nav-tabs .active').removeClass('active');
		$('.hobbes-syncs-nav-tabs .nav-tab-active').removeClass('nav-tab-active');
		...
		$('.hobbes-syncs-editor-panel').hide();
		$( tab_id ).show();
		
		if( hsyncc_code_editor ){
			hsyncc_code_editor.toTextArea();
			hsyncc_code_editor = false;
		}

		if( $( tab_id ).find('.hobbes-syncs-code-editor').length ){

			hsyncc_init_editor( $( tab_id ).find('.hobbes-syncs-code-editor').prop('id') );
			hsyncc_code_editor.refresh();
			hsyncc_code_editor.focus();
		}

		jQuery('#hobbes-syncs-active-tab').val(tab_id).trigger('change');
	...
	window.onbeforeunload = function(e) {
		...
		if( hobbes_syncs_canvas && hobbes_syncs_canvas !== jQuery('#hobbes-syncs-live-config').val() ){
			return true;
		}
	};
...
function hsyncc_init_editor(el){
	if( !jQuery('#' + el).length ){
		return;
	}	
	// custom modes
	var mustache = function(hobbes_syncs, state) {

		var ch;

		if (hobbes_syncs.match("{{")) {
			while ((ch = hobbes_syncs.next()) != null){
				if (ch == "}" && hobbes_syncs.next() == "}") break;
			}
			hobbes_syncs.eat("}");
			return "mustache";
		}
		...
		while (hobbes_syncs.next() != null && 
			//!hobbes_syncs.match("{", false) && 
			!hobbes_syncs.match("{{", false) && 
			!hobbes_syncs.match("%", false) ) {}
			return null;
	};
	...
	hsyncc_code_editor = CodeMirror.fromTextArea(document.getElementById(el), options);
	hsyncc_code_editor.on('keyup', tagFields);
	hsyncc_code_editor.on('blur', function(cm){
		cm.save();
		jQuery( cm.getInputField() ).trigger('change');
	});

	return hsyncc_code_editor;

}
(function() {
	"use strict";

	if( typeof CodeMirror === 'undefined' || hobbes_syncs_canvas === false ){
		return;
	}
			...
			if(token.type){
				...
				if( token.type ){
					...
					if( in_entry ){
						...
						jQuery('.hobbes-syncs-autocomplete-in-entry-' + token.type).each(function(){
						...
						jQuery('.hobbes-syncs-autocomplete-out-entry-' + token.type).each(function(){
							var field = jQuery(this);
							fields[field.data('slug')] = field.data('label');
							//fields["#each " + field.data('slug')] = "#each " + field.data('label');
							//if( !in_if ){
